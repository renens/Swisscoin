...
function calcFee(value, withDecimals) {
  const defaultFee = value / 100;
  const maxFee = withDecimals ? 250 * Math.pow(10, 18) : 250;
...
function findMax(initValue, balance, withDecimals) {
  const f = function (x) {
    return balance - x - calcFee(x, withDecimals);
  };
  let initFee = calcFee(initValue,withDecimals)*2;
  var a = initValue+ initFee;
  var b = initValue - initFee;
...
function transferMax(balance, withDecimals) {
  var calcMax=function (a) {
    let maxFee = calcFee(a, withDecimals);
    return a-maxFee
  };

  let maxValue = calcMax(balance);
  let fee=calcFee(maxValue,withDecimals);

  if(maxValue+fee===balance){
    return maxValue;
  }
  else{
    return findMax(maxValue, balance, withDecimals);
  }
