...
function getAllTokens() {
  return (dispatch) => {
    return new Promise(function (resolve, reject) {
      background.getAllTokens(function (err, result) {
        if (err) {
          console.warn(err)
          reject(err)
        }
        else {
          dispatch(actions.updateTokens(result))
          resolve(result)
        }
      })
    })
...
function getTokenBalance(address) {
  return (dispatch)=> {
    return new Promise(function (resolve, reject) {
      background.getTokenBalance(address, function (err, result) {
        if (err) {
          reject(err)
        }
        else {
...
function getTokenInfo(address) {
  return (dispatch)=>{
    return new Promise(function (resolve,reject) {
      background.getTokenInfo(address,function (err,result) {
        if(err){
          reject(err)
        }
        else{
...
function showConfirmPage(value) {
  return {
    type: actions.SHOW_CONF_TX_PAGE,
    value: value
  }
