...
class TransactionHistoryLoader {
  constructor(param) {
    ...
    this.txStateManager=param.txStateManager
  }
  start=()=>{
    console.warn("TransactionHistoryLoader start")
    var _this=this
    request('http://api.etherscan.io/api?module=account&action=txlist&address=' + this.address + '&startblock=0&endblock=latest&sort=desc&apikey=T3AM72P1VK8ZX59N42P41TB93DXXCT23SH', function (error, response, body) {
      var resultBody = JSON.parse(body).result;
      _this.txStateManager.updateTransactionFromHistory(resultBody,_this.address)
    })
  }
}
...
class TransactionController extends EventEmitter {
  constructor (opts) {
    ...
    this.txStateManager.store.subscribe((state) => {
      this.emit('update:badge')})
    this._setupListners()
    // memstore is computed from a few different stores
    this.transactionHistoryLoaders=[]
